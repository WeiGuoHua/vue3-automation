  ...
  <div v-loading="loading">
    ...
    >
      ...
      <template v-if="confirmBtn || cancelBtn">
        <el-button type="primary" v-if="confirmBtn" @click="submit">{{ confirmBtn }}</el-button>
        <el-button v-if="cancelBtn" @click="cancelClick">{{ cancelBtn }}</el-button>
      </template>
    </el-form>
  </div>
</template>
...
const forEachGetFormModel = (list: FormList[], obj: any) => {
  list.forEach((item: any) => {
    if (['table', 'flex'].includes(item.type)) {
      ...
      if (item.config?.transform && !transformField.value.includes(item.name)) {
...
const submit = () => {
  ...
  if (confirmBtn.value && props.submitUrl !== false && loading.value === false)
    validate((valid: boolean, fields: any) => {
      if (valid) {
        ...
        console.log(props.submitUrl, params);
        // 提交保存表单
